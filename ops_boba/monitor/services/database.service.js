...
class DatabaseService extends OptimismEnv {
  ...
  async getNewestBlockFromStateRootTable() {
    ...
    const latestBlock = await query(
      `SELECT MAX(stateRootBlockNumber) from stateRoot`
    )
    con.end()
    return latestBlock
  }
  ...
  async getPendingMessageFromReceiptTable() {
    ...
    const pendingMessages =
      await query(`SELECT receipt.hash, receipt.blockNumber, receipt.from, receipt.to,
  ...
  async getNewestBlockFromLayerZeroTx(chainID) {
    const con = mysql.createConnection({
      host: this.MySQLHostURL,
      port: this.MySQLPort,
      user: this.MySQLUsername,
    ...
    await query(`USE ${this.MySQLDatabaseName}`)
    const latestBlock = await query(
      `SELECT MAX(blockNumber) from layerZeroTx where chainID = ${chainID}`
    )
    con.end()
