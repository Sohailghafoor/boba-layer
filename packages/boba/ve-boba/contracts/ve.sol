...
contract ve is IERC721, IERC721Metadata {
    ...
    string constant public name = "veBOBA";
    string constant public symbol = "veBOBA";
    ...
    function _find_timestamp_user_epoch(uint _tokenId, uint _timestamp, uint max_user_epoch) internal view returns (uint) {
        uint _min = 0;
        uint _max = max_user_epoch;
        for (uint i = 0; i < 128; i++) {
            if (_min >= _max) break;
            uint _mid = (_min + _max + 2) / 2;
            Point memory pt = user_point_history[_tokenId][_mid];
            if (pt.ts <= _timestamp) {
                _min = _mid;
            } else {
                _max = _mid -1;
    ...
    /// @notice Get the voting power of `_tokenId` at `_timestamp`
    /// @param _tokenId NFT for lock
    /// @param _timestamp time to return voting power at
    function ve_for_at(uint _tokenId, uint _timestamp) external view returns (uint) {
        uint max_user_epoch = user_point_epoch[_tokenId];
        uint _epoch = _find_timestamp_user_epoch(_tokenId, _timestamp, max_user_epoch);
        Point memory pt = user_point_history[_tokenId][_epoch];
        pt.bias -= pt.slope * (int128(int256(_timestamp - pt.ts)));
        if (pt.bias < 0) {
            pt.bias = 0;
        }
        return uint(int256(pt.bias));
    }
    ...
    // NOTE: extrapolation of blocknumber might not work precisely with varying block time in L2
    // Consider using timestamp checkpoints for historical balance/voting power
    // This will be removed in the future if this is unused
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) internal pure returns (string memory output) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "ve-Boba locks can be used to vote on token emission, and the community DAO", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
